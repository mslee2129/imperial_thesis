...
class CCGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G_A', 'G_B', 'D_A', 'D_B', 'cycle_A', 'idt_A']
    ...
    def backward_G_A(self):
        """Calculate GAN and L1 loss for generator A"""
        # First, G(A) should fake the discriminator
        fake_AB = torch.cat((self.real_A, self.fake_B), 1)
        pred_fake = self.netD_A(fake_AB)
        self.loss_G_GAN = self.criterionGAN(pred_fake, True)
        # Second, G(A) = B
        self.loss_G_L1 = self.criterionCycle(self.fake_B, self.real_B) * self.opt.lambda_L1
        # combine loss and calculate gradients
        self.loss_G_A = self.loss_G_GAN + self.loss_G_L1
        self.loss_G_A.backward()

    def backward_G_B(self):
        """Calculate the loss for generator and G_B"""
        ...
        self.loss_G_B = self.loss_G_B + self.loss_cycle_B + self.loss_idt_A
